...
Exercise 14:
	 chown betty:school *
	 Change owner to `betty` and the group owner to `school` for all files and directories in current directory.

Exercise 15:
	 chown -h betty:school _hello
	 Changes the owner and group owner of file `_hello` to `betty` and `school` respectively.
